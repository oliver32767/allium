 ...
 */
package io.firstwave.allium.gen.math;
...
import static io.firstwave.allium.gen.math.Numbers.factorial;
...
public class BigNumbers {
     ...
     */
    public static MathContext mathcontext = new MathContext(PRECISION, RoundingMode.HALF_EVEN);
    /**
    ...
    /**
     * The number 1/6 as a BigDecimal. It is computed with the initial value of {@link #mathcontext}.
     */
    public static final BigDecimal ONE_SIXTH = ONE_DOT.divide(BigDecimal.valueOf(6), mathcontext);
    /**
     * The number 1/3 as a BigDecimal. It is computed with the initial value of {@link #mathcontext}.
     */
    public static final BigDecimal ONE_THIRD = ONE_DOT.divide(BigDecimal.valueOf(3), mathcontext);
    /**
     ...
     * See <a href="http://www.research.att.com/~njas/sequences/A002117" target="_top">http://www.research.att.com/~njas/sequences/A002117</a>,
     ...
     * that is, the integers <i>p, q</i> such that <i>x</i> &#x2248 <i>p/q</i>.
     ...
     * <code>y[1]</code> = <i>q</i> such that <i>x</i> &#x2248 <i>p/q</i>
     ...
     * according to the context {@link #mathcontext}.
    ...
    public static BigDecimal root(int n, BigDecimal z) {
        ...
        if (n < 0) return ONE_DOT.divide(root(-n, z), mathcontext);
     ...
     *
     * @param n the integer to isOverlap
     ...
     * This algorithm is the core of the Miller-Rabin primality isOverlap,
     ...
     * @param a the base of the strong probable primality isOverlap
    ...
    /**
     *  The AKS primality isOverlap, returns true if the integer <i>n</i> > 1 is prime.
     *  This isOverlap has a polynomial time complexity with respect to log <i>n</i>,
   ...
   */
     ...
     * <a href="http://www.research.att.com/~njas/sequences/A013631" target="_top">http://www.research.att.com/~njas/sequences/A013631</a>,
     * for &#x03C0; up to the limit 43, cf.
     * <a href="http://www.research.att.com/~njas/sequences/A001203" target="_top">http://www.research.att.com/~njas/sequences/A001203</a>.
     *
   ...
   public static BigDecimal ln2(BigDecimal x, int n) {
      int precision = 100;
      if (x.signum() <= 0) throw new IllegalArgumentException("ln(" + x.setScale(5,RoundingMode.HALF_EVEN)+") not defined");
      // Determine maximum m such that e^m <= x (where m = 0 if x < 1):
      int m = 0;
      while (E.pow(m+1).compareTo(x) < 0) { m++; }
      // x = x / e^m:
      //x = x.divide(E.pow(m), mathcontext); // <- does not modify the scale of x!!
      x = x.divide(E.pow(m), precision, RoundingMode.HALF_EVEN);
      // y = (x-1) / (x+1):
      BigDecimal y = x.subtract(ONE_DOT).divide(x.add(ONE_DOT), RoundingMode.HALF_EVEN);
   ...
   public static void main (String[] args) {
      ...
      //BigDecimal root_10_2u = new BigDecimal("1.071773462536293164213006325023342022906384604977556784");
      //BigDecimal root_10_2b = new BigDecimal("1.071773462536293164213006325023342022906384604977556783");
      //System.out.println(root_10_2u.pow(10).setScale(55, BigDecimal.ROUND_HALF_UP));
      //System.out.println(root_10_2b.pow(10).setScale(55, BigDecimal.ROUND_HALF_UP));
      //System.out.println(root_10_2u.scale());

      //--- root: -----------
      //BigDecimal x = TWO_DOT, y;
      //int n = 5;
      //for (int i=0; i <= 100; i += 10) {
      //   y = x.multiply(TEN_DOT.pow(i));
      ...
      //   y = x.divide(TEN_DOT.pow(i), mathcontext);
      //   System.out.println(y.setScale(PRECISION,BigDecimal.ROUND_HALF_EVEN) + " =\n" + root(n,y).pow(n).setScale(PRECISION,BigDecimal.ROUND_HALF_EVEN));
      //}
      ...
      //--- AKS: -----------
      // boolean prime;
      // BigInteger m = new BigInteger("1524157877488187891"); // needs about 260 sec = 4:20 min
      ...
      // prime = primalityTestAKS(m);
      // time = System.currentTimeMillis() - time;
      // System.out.println(m + " prime? " + prime + " (running time AKS: " + time + " ms)");
      //
      // modulo:------------
...
//      }
      ...
      for (BigDecimal i = BigDecimal.valueOf(1.); i.compareTo(BigDecimal.valueOf(2.1)) < 0; i = i.add(BigDecimal.valueOf(.1))) {
         System.out.println(decToBin(i,10) + " = " + Numbers.decToHex(i.doubleValue(),10));
      }
      // ------------------------------------
...
//       System.out.println(x + "^" + e + " mod " + n + " = "+modPow(x,e,n));
      ...
      //System.out.println(isOverlap + " = " + decToHex(isOverlap,50) + "_{16}");
      //
